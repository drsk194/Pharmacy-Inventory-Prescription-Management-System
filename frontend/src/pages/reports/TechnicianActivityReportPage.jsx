...
export default function TechnicianActivityReportPage() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await reportApi.getTechnicianActivity({
        startDate: dates.startDate || undefined,
        endDate: dates.endDate || undefined,
      });
      setRows(res.data.data || []);
    } catch (err) {
      setError(err.response?.data?.message || "Could not load technician activity.");
    }
  }, [dates]);
  ...
  return (
    <main className="list-page report-page">
      ...
      <div className="stat-grid" style={{ margin: "1rem 0 1.5rem" }}>
        ...
        <StatCard tone="emerald" label="Technicians active" value={rows.length} />
      ...
      {chartData.length > 0 && (
        <div className="chart-card" style={{ marginBottom: "1.1rem" }}>
          <div className="chart-card__header">
            <div>
              <p className="chart-card__title">Prescriptions processed per technician</p>
          ...
          <ResponsiveContainer width="100%" height={Math.max(200, chartData.length * 42 + 40)}>
            <BarChart data={chartData} layout="vertical" margin={{ top: 4, right: 56, left: 8, bottom: 0 }}>
              ...
              <Bar dataKey="count" name="Prescriptions processed" radius={[0, 6, 6, 0]} maxBarSize={26}>
      ...
      <table className="data-table">
        <thead>
          <tr><th>Technician</th><th>Prescriptions processed</th><th>Share</th></tr>
        </thead>
        <tbody>
          {rows.map((r) => (
            <tr key={r.label}>
